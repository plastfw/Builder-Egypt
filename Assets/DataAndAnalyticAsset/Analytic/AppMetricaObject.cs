...
public class AppMetricaObject : ScriptableObject
{
    ...
    public void OnSoftSpend(string type, string name, float amount, int count)
    {
    ...
    public void OnGameExit(string registrationDate, int sessionCount, int daysInGame, float currentSoft)
    {
        AppMetrica.Instance.ReportEvent("reg_day", new Dictionary<string, object>()
        {
            {"date", registrationDate }
